...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
			# Update beliefs except for jail position to 0 and update belief of jail position to 1 when ghost is captured.
			...
			if emissionModel[trueDistance] > 0:
				# P(y/x) = P(x/y) * p(y) // Approximate bayes rule update
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
        	...
        	# Obtaining the future distribution from the old state.
        	newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        	
        	# Calculating beliefs of new state based on the new distribution and the previous step belief.
        	# self.getBeliefDistribution() -> old beliefs
        	# newPosDist -> Probability distribution based on the new state of the ghost
        	for newPos, prob in newPosDist.items():
        		# Adding because, the newPos can occur more than once. Hence, it's probability increases for each time \
        		# newPos occurs. So hence, we add the probability, for each time the same newPos occurs.
        		allPossible[newPos] += self.getBeliefDistribution()[oldPos] * newPosDist[newPos]

        self.beliefs = allPossible

    def getBeliefDistribution(self):
    	
    	#we have to normalize the beliefs as all values must add up to 1
    	self.beliefs.normalize()
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #Distributing particles evenly in all positions. Each position will recieve NumParticles/NumPositions particles.
        self.particleList = []
    ...
    def observe(self, observation, gameState):
        ...
        #For all other cases when ghost is not captured
        if noisyDistance is not None:
        	currentBeliefs = self.getBeliefDistribution()

        	#For each position update the current beliefs based on P(noisyDistance/Truedistance from the pacman position) which is the weight for each particle.
        	for position in self.legalPositions:
        		trueDistance = util.manhattanDistance(position,pacmanPosition)
        		
        		# P(Y) = weight * P(X)
        		currentBeliefs[position] = emissionModel[trueDistance] * currentBeliefs[position]
        		
        		#Checking if all particles receive weight 0.
        		if currentBeliefs[position] > 0:
        			isEmpty = False
        	
        	#If each particle receive weight 0, we need to uniformly distibute the particles.
        	if isEmpty:
        		self.initializeUniformly(gameState)
        	
        	# Resampling the particles from the computed weighted beliefs
        	else:
        		for i in range(self.numParticles):
        			newParticleList.append(util.sample(currentBeliefs))
        
        # For ghost being captured, all particles should be present at the jail position
        else:
        	for particle in range(self.numParticles):
        		newParticleList.append(self.getJailPosition())

        if not isEmpty or noisyDistance is None:	
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        newParticleList = [] #list of particles in next state
        for oldPosParticle in self.particleList:
        	newGameState = self.setGhostPosition(gameState,oldPosParticle)
        	newParticleBeliefs = self.getPositionDistribution(newGameState)

        	#sampling next state for each particle based on the newPos distribution; y = sample(P(Y/x))
        	#newParticleBeliefs = P(Y/x)
        	#y = next state
        	newParticleList.append(util.sample(newParticleBeliefs))
    ...
    def getBeliefDistribution(self):
        ...
        # Belief distribution is nothing but the no_of_particles_in_each_position/total_no_of_particles
        # More number of particles, higher the probability of the ghost being present there.
        beliefDistribution = util.Counter()
        for particle in self.particleList:
        	beliefDistribution[particle] += 1.0/self.numParticles # Probability of each particle is 1/numParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        allPossibleParticles = []
        # Obtaining the set of all possible permutations of the set of LEGAL ghost positions and shuffling it each time to avoid the ordering due to itertools.product
        for particle in itertools.product(self.legalPositions,repeat=self.numGhosts):
        	allPossibleParticles.append(particle)
        	random.shuffle(allPossibleParticles)
        
        # From the set of all SHUFFLED possible particles, we randomly choose each particle to be our set of particles. 
        #The shuffling and random chosing, helps us evenly distribute the particles.
        self.particleList = []
        for i in range(self.numParticles / len(allPossibleParticles)):
        	self.particleList += allPossibleParticles
    ...
    def observeState(self, gameState):
        ...
        # Like in particle filter, we are now calculating the weighted particle distribution from the weights and the unweighted \
        # belief distribution of particles.
        # Joint_weighted_belief = weight(ghost 1) * weight(ghost 2) * ...... * weight(ghost n) * joint_unweighted_belief \
        # Mulitplying because, weight is the probability of noisy distance given true distance, hence by using product rule we obtain the above.
        for particle in currentBeliefs:
			for ghostIndex in range(self.numGhosts):
				noisyDistance = noisyDistances[ghostIndex]
				if noisyDistance is not None:
					currentBeliefs[particle] = emissionModels[ghostIndex][util.manhattanDistance(pacmanPosition, particle[ghostIndex])] * currentBeliefs[particle]
			if currentBeliefs[particle] > 0:
				isEmpty = False

        # Handlint he case where each particle recieves weight of 0
        if isEmpty:
        	self.initializeParticles()
        # Resampling the weighted particle distribution
        else:
        	newParticleList = []
        	for i in range(self.numParticles):
        		sample = util.sample(currentBeliefs)
        		newParticleList.append(sample)
			self.particleList = newParticleList

		# Updaating the distribution to set the ghosts in Jail if captured
		newParticleList = []
		for particle in self.particleList:
			for ghostIndex in range(self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particleList:
            ...
            # Sampling the new particle by considering the future position of each ghost, from each of it's old position(Particle Tuple)
            for ghostIndex in range(self.numGhosts):
            	newParticle[ghostIndex] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState,oldParticle), ghostIndex, self.ghostAgents[ghostIndex]))

            newParticleList.append(tuple(newParticle))
        self.particleList = newParticleList

    def getBeliefDistribution(self):
		# Similar to the getBeliefDistribution() in ParticleFilter, the belief distribution in this case \
		# would be nothing but the no_of_particles_in_each_position_tuple/total_no_of_particles.
		# So, similarly, more the number of particles per position tuple, higher is the probability \
		# that the ghosts are located in the locations specified in the position tuple.
